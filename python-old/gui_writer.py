...
class OrderBookWriter(Frame):
	...
	def push_tx(self, app_info, order):
		...
		signed_tx = self.broadcaster.sign_tx(self.broadcaster.make_tx(order,
			self.keys['pub']), self.keys['priv'])
		pushed_tx = self.broadcaster.push_tx(signed_tx)
		pyp.copy(pushed_tx['id'])
		print(self.time_tx(pushed_tx['id'], order['metadata']['timestamp']))
		return pushed_tx['id']
